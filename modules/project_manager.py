...
def smart_geocode(address_input):
    ...
    try:
        ...
        # 4. STATIC FALLBACK (Last Resort if API fails or returns nothing)
        # Check potential commune found in step 2
        if potential_commune:
            static_match = check_static(potential_commune)
            if static_match:
                return static_match
        
        # Check explicit input against static
...
def render_project_details(project_id):
    ...
    if st.button("📄 Generar Ficha de Proyecto", key="btn_export_pdf", help="Generar informe ejecutivo del proyecto"):
        with st.spinner("Generando Ficha de Proyecto..."):
            ...
            if not phases.empty:
                ...
                legend_patches = [
                    mpatches.Patch(color='#10b981', label='Completada'),
                    mpatches.Patch(color='#3b82f6', label='En Progreso'),
                    mpatches.Patch(color='#ef4444', label='Atrasada'),
                    mpatches.Patch(color='#cbd5e1', label='Pendiente')
                ]
                ax2.legend(handles=legend_patches, loc='upper right', fontsize='small')
    ...
    with tabs[0]:
        ...
        if not phases.empty:
            ...
            phases['start_date'] = pd.to_datetime(phases['start_date'])
            phases['end_date'] = pd.to_datetime(phases['end_date'])
            # Sort by ID to maintain insertion order (user request) and fix alignment stability
            phases = phases.sort_values(by=['id'], ascending=True)
            ...
            # Dynamic Height: Compact (35px per item)
            dyn_height = max(350, len(phases) * 35 + 100)
