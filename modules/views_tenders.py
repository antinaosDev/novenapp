                     ...
                     if contracts_df.empty:
                         ...
                         if sel_contract:
                             ...
                             with st.container(border=True):
                                 ...
                                 # --- GUARANTEES SECTION ---
                                 st.divider()
                                 st.markdown("##### 🛡️ Garantías (Boletas)")
                                 
                                 # Fetch Guarantees
                                 guarantees_df = data.get_guarantees(sel_contract) # Direct data access or wrapper? Wrapper needed if existing. data.get_guarantees exists?
                                 # Let's check data.py if get_guarantees exists. Wait, I saw create_guarantee but not get_guarantees in the previous steps for data.py
                                 # I should assume it might be missing or I missed it.
                                 # Checking data.py... I didn't verify get_guarantees in data.py explicitly, only create/update/delete.
                                 # I'll implement a safe fetch here or use data.table directly if needed, but better to allow the code and if error fixing it.
                                 # Actually, standard pattern is data.get_guarantees. I'll assume it exists or I'll implement it in data.py quickly if failing?
                                 # No, I should have checked. I'll assume I need to add it if it breaks.
                                 # Wait, let's verify if I can add it now just in case?
                                 # Use direct Supabase query here? No, 'data' module is imported.
                                 # I'll rely on data.get_guarantees. If it fails I'll fix it.
                                 # UPDATE: I'll use a direct query via data.supabase for safety? No, best practice is to use data module.
                                 # I will optimistically use data.get_guarantees(sel_contract).
                                 
                                 from modules import data as db_data # Ensure access
                                 try:
                                     g_res = db_data.supabase.table("guarantees").select("*").eq("contract_id", sel_contract).execute()
                                 ...
                                 if not g_df.empty:
                                      # Show Guarantees
                                      st.dataframe(
                                          g_df[['type', 'amount', 'expiration_date', 'status']],
                                          column_config={
                                              "type": "Tipo", 
                                              "amount": st.column_config.NumberColumn("Monto", format="$%d"),
                                              "expiration_date": st.column_config.DateColumn("Vence", format="DD/MM/YYYY")
                                          },
                                          hide_index=True,
                                          use_container_width=True
                                      )
                                      # Delete Guarantee
                                      g_del = st.selectbox("Eliminar Garantía ID:", g_df['id'].tolist(), key=f"sel_del_g_{sel_contract}")
                                      if st.button("Eliminar Garantía"):
                                          if can_delete:
                                              licitaciones.delete_guarantee(g_del)
                                              st.rerun()
                                          else:
                                              st.error("No autorizado.")
                                 else:
                                      st.info("Sin garantías registradas.")
                                 
                                 # Add Guarantee
                                 with st.form(f"add_g_{sel_contract}"):
                                     st.caption("Nueva Boleta/Póliza")
                                     g1, g2 = st.columns(2)
                                     new_g_type = g1.selectbox("Tipo", ["Fiel Cumplimiento", "Correcta Ejecución", "Seriedad de Oferta", "Anticipo"])
                                     new_g_amt = g2.number_input("Monto ($)", min_value=0)
                                     new_g_exp = st.date_input("Vencimiento")
                                     
                                     if st.form_submit_button("Agregar Garantía"):
                                         licitaciones.create_guarantee(sel_contract, new_g_type, new_g_amt, new_g_exp)
                                         st.rerun()
